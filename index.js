...
function artistInfo(artistArray, artistName) {
  for (let i = 0; i < artistArray.length; i++) {
    if (artistArray[i].name === artistName) {
...
function artistByCountry(artistArray, nationality) {
  ...
  for (let i = 0; i < artistArray.length; i++) {
    let artistNationalities = artistArray[i].nationality.split(', ');
    if (artistNationalities.includes(nationality) && artistNationalities.length === 1) {
      resultArray.push(artistArray[i].name);
    }
  }
